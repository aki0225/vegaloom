...
class GoalHandoffWorkspaceBinding(BaseModel):
    ...
    @field_validator("changed_files", "untracked_files")
    @classmethod
...
def create_goal_handoff(
    ...
    *,
    ...
    version_dir = _prepare_version_dir(run_dir, checkpoint, version)
    ...
    _write_text_atomic(
        ...
        _render_handoff_report(
            ...
            handoff_path.relative_to(run_dir).as_posix(),
        ),
    )
    artifact_paths = [
        handoff_path.relative_to(run_dir).as_posix(),
...
def compile_goal_handoff_context(
    *,
    ...
    consumer_dir = version_dir / "consumers" / normalized_session
    _prepare_consumer_dir(consumer_dir, version_dir, run_dir)
    ...
    metrics = {
        ...
        "section_chars": [
            {"section": name, "chars": len(text.rstrip() + "\n")}
    ...
    if context_chars > max_chars:
        ...
        artifacts = [
            metrics_path.relative_to(run_dir).as_posix(),
            split_json_path.relative_to(run_dir).as_posix(),
            split_report_path.relative_to(run_dir).as_posix(),
        ]
    ...
    artifacts = [
        ...
        context_json_path.relative_to(run_dir).as_posix(),
        metrics_path.relative_to(run_dir).as_posix(),
    ]
    return GoalHandoffCompileResult(
        status="ready",
...
def _bind_authoritative_artifacts(
    ...
    candidates = [
        GoalHandoffArtifactInput(
            scope="goal_run",
            path=f"checkpoints/{checkpoint_id}/checkpoint-evidence.json",
        ),
        GoalHandoffArtifactInput(
            scope="goal_run",
            path=f"checkpoints/{checkpoint_id}/checkpoint-report.md",
        ),
        *requested,
    ]
    ...
    for item in candidates:
        ...
        if key in seen:
            continue
        seen.add(key)
        root = run_dir if item.scope == "goal_run" else repo
        path = _resolve_bound_file(root, item.path, item.scope)
        bindings.append(
...
def _validate_bound_artifacts(
    ...
    *,
    ...
    for artifact in artifacts:
        root = run_dir if artifact.scope == "goal_run" else repo
    ...
    for current_root, directory_names, filenames in os.walk(
        ...
        for directory_name in directory_names:
            ...
            if _is_link_or_reparse_stat(metadata):
                # Policy 扫描不跟随目录 alias；显式 artifact 路径会单独 fail-closed。
                continue
...
def _build_split_plan(
    ...
    for item in section_items:
        item_chars = int(item["chars"])
        if current_names and current_chars + item_chars > max_chars:
            groups.append(
                {
                    "sections": current_names,
                    "chars": current_chars,
            ...
            current_names = []
            current_chars = 0
        current_names.append(str(item["section"]))
...
def _prepare_version_dir(
    ...
    try:
        ...
        raise ValueError(f"handoff version 已存在，不能覆盖：{version}") from exc
    _require_existing_directory_within(run_dir, version_dir, "handoff version")
    return version_dir
...
def _prepare_consumer_dir(
    ...
    try:
        consumer_dir.mkdir(exist_ok=False)
    except FileExistsError as exc:
        raise ValueError(
            f"consumer session artifact 已存在，不能覆盖：{consumer_dir.name}"
        ) from exc
    _require_existing_directory_within(
        run_dir,
        consumer_dir,
...
def _resolve_bound_file(root: Path, relative: str, label: str) -> Path:
    ...
    try:
        # 先检查调用方提供的原始路径，避免 resolve 后丢失 alias 身份。
