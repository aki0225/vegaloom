...
Vega 是本地优先的 AI 编码工作流 harness:worker 改代码,reviewer 在隔离上下文里只看 diff 与测试证据;任务能否结束由项目自己的验证命令裁决,证据不足时 fail-closed 交还人工。
...
- `loops/` — 运行产物目录(state/trace/报告落盘处)
...
3. **写审隔离不得打通**——reviewer 只接收 diff、测试证据和项目规则,不得为了"提升效果"把 worker 的对话内容、自述或中间推理喂给 reviewer。
