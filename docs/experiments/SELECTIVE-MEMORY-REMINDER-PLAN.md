...
- 状态：待最终评审，尚未授权实施。
- 日期：2026-07-12。
- 当前动作：只维护计划，不创建实验分支，不修改 runtime，不运行真实 LLM 实验。
- 实施前提：先完成 v0.1.0 开源前收口，并在 `main` 上创建可复现的 `v0.1.0` tag。
- 实验基线：仅在用户确认实施后，才从 `v0.1.0` tag 创建实验分支，并在 Phase 0 报告中记录准确 commit SHA。
- 主线原则：实验不得改变 v0.1.0 的默认行为、成功条件、退出码或必需 artifacts。
- 停止线：Phase 2 完成后必须停止，只有用户再次明确授权才能进入 Phase 3。
...
在这些边界获得明确确认前，本计划保持“待最终评审”，不得开始实现。
