# Supervisor Agent Gate 2B 实施前冻结
...
1. 创建当前 Work Item 的 assist child loop；
...
- 单个 Work Item 最多一次初始 attempt 和一次人工明确批准的 repair attempt；
- 每个 Agent run 最多一次 replan；超过后转 `needs_human`。

attempt 和 replan 上限从现有 Trace 计数，不增加另一套预算状态对象。本轮也不新增 Token
计费器；若 Codex JSONL 提供 usage，只记录为诊断信息，不用它决定成功。
...
- `src/vega/agent_*`：薄 Codex Adapter、机器 Observation 和状态卡；
...
3. 外部 Claim 不能升级为机器 Observation；
4. 真实 Adapter 的 Observation 必须引用 execution、Workspace 和 child Core Artifact；
5. stop 只作用于当前 child 的 owned process；
6. timeout、无效 JSON、缺少最终消息和 termination-unconfirmed 都不启动后续 Gate；
7. Worker 与 Reviewer 命令、Prompt 和完整聊天保持隔离；
8. `do / loop / goal` 回归保持不变。
...
本文批准后才创建一个 Gate 2B 短生命周期实验分支和一个专用 Worktree。未批准前只允许修正文档
事实错误，不开始 Runtime 改造。
