...
> 状态：历史设计记录。P0 人工状态层、P1 单 checkpoint、child reconcile 和人工显式
> `--rerun-worker` 曾进入主线；`v0.3.0` 已删除公开的 `vega goal` 入口，长任务统一由顶层
> ChangeRun 命令驱动。本文保留当时的设计、实验和判断，不作为当前命令说明。
>
> 阅读说明：本文后半部分按时间保留 r3～r6、审阅修复和 PR CI 前后的原始判断。带日期章节
> 中的“当前分支”“等待 PR CI”等表述是当时证据，不代表当前状态。
