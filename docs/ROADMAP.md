...
> 合并前审阅，状态为 `gate-exit-pass`。Gate 3B 已获批准，当前先完成固定控制器和未知
> 副作用继承前置门禁；Gate 3C 仍冻结。既有
...
v0.1.5 发布（完成）
  ...
  -> Gate 3B 单 Work Item 跨机器接力（前置门禁修复中）
...
`gate-exit-pass`。Gate 3B 随后获批，当前先修复固定控制器和未知副作用继承前置门禁；
Gate 3C 日常价值观察仍冻结。
