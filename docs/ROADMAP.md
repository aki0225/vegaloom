...
> 更新时间：2026-08-20
> 当前稳定基线：`v0.2.0`
> 当前发布候选：`v0.2.1`；远端 Tag 与 GitHub Release 尚未创建
...
### 2026-08-19：v0.2.1 维护发布候选

`v0.2.1` 不增加新 Runtime、Agent 角色或成功路径。本轮只收紧 V1 批准前的单 Work Item 与
非空允许路径校验，完善 Agent 可选依赖探测，并对齐目标仓库工作目录、Core Finish 与父 Agent
finalize、Git-only fresh-clone 证据边界、Reviewer 隔离声明和安全支持版本。发布仍要求同一
提交通过完整 CI、wheel/sdist smoke 与发布复核；完成前不将 v0.2.1 记为稳定或已发布版本。
